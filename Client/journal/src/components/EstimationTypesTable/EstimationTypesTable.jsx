...
import {
    ...
    setLimit,
    setSearchParams
} from '../../store/slices/estimationTypeSlice';

const EstimationTypesTable = () => {
    ...
    const {
        ...
        meta,
        searchParams
    } = useSelector(state => state.estimationTypes);
    ...
    const handleSaveEdit = async () => {
        if (!editType.name?.trim()) {
            showAlert('Название типа оценки должно быть заполнено!', 'error');
            return;
        }

        try {
            await dispatch(updateEstimationType({
                id: editType.id,
                typeData: { name: editType.name }
            })).unwrap();

            showAlert('Тип оценки успешно обновлен!', 'success');
            handleCloseModals();
            dispatch(fetchEstimationTypes({
                limit: meta.limit,
                page: meta.page,
            ...
            showAlert(error.message || 'Ошибка при обновлении типа оценки', 'error');
        }
    };
    ...
    const handleSaveAdd = async () => {
        ...
        try {
            await dispatch(addEstimationType({ name: newType.name })).unwrap();
            showAlert('Тип оценки успешно добавлен!', 'success');
            handleCloseModals();
            dispatch(setPage(1));
            dispatch(fetchEstimationTypes({
                limit: meta.limit,
            ...
            showAlert(error.message || 'Ошибка при добавлении типа оценки', 'error');
        }
    };

    const handleDeleteConfirm = async () => {
        try {
            await dispatch(deleteEstimationType(currentRow.id)).unwrap();
            showAlert('Тип оценки успешно удален!', 'success');
            handleCloseModals();

            if (typesData.length === 1 && meta.page > 1) {
            ...
            showAlert(error.message || 'Ошибка при удалении типа оценки', 'error');
        }
    };
        ...
        <>
            <TableContainer component={Paper}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 2 }}>
                    <Typography variant="h6">Типы оценок</Typography>
            ...
            <Modal open={openEditModal} onClose={handleCloseModals}>
                <Box sx={{
                    ...
                    <Box sx={{
                        ...
                        <Typography variant="h6">Редактировать тип оценки</Typography>
            ...
            <Modal open={openDeleteModal} onClose={handleCloseModals}>
                <Box sx={{
                    ...
                    <Box sx={{
                        ...
                        <Typography variant="h6">Удалить тип оценки</Typography>
                        <IconButton onClick={handleCloseModals} sx={{ color: 'white' }}>
                            <CloseIcon />
                        </IconButton>
                    </Box>
                    <Box sx={{ p: 3 }}>
                        <Typography>Вы уверены, что хотите удалить тип оценки "{currentRow?.name}"?</Typography>
                        <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 2 }}>
                            <Button onClick={handleCloseModals}>Отмена</Button>
                            <Button
            ...
            <Modal open={openAddModal} onClose={handleCloseModals}>
                <Box sx={{
                    ...
                    <Box sx={{
                        ...
                        <Typography variant="h6">Добавить новый тип оценки</Typography>
                        <IconButton onClick={handleCloseModals} sx={{ color: 'white' }}>
                            <CloseIcon />
                        </IconButton>
                    </Box>
...
export default EstimationTypesTable;