...
export const getStudentSubjects = createAsyncThunk(
  "curriculums/getStudentSubjects",
  async (params, { rejectWithValue }) => {
    try {
      const response = await CurriculumService.getStudentSubjects(params);
      return response;
    } catch (error) {
      return rejectWithValue(error.response?.data || error.message);
    }
...
const curriculumSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      //get student subejcts
      .addCase(getStudentSubjects.pending, (state, action) => {
        state.isLoading = true;
        state.currentStudentSubjects = [];
      })
